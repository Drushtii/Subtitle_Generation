...
The script can be run on CPU or GPU. It is recommended to use base or medium models to run the script on CPU.

### Installation

1. Install requirements with command `pip install .` from the repository root
...
`python3 <```main.py file path```> --model_size base --lang hindi --input_file <```input file path```> --output_file output/largeenglish.srt`
